...
#and image size must be even
    ...
    def __getData(self,root):
        ...
        for directory in self.__directoryList:
          for filenames in os.listdir(directory +'/blur'):
            #for filename in filenames:
              self.__fileBlurList.append(directory+'/blur/'+filenames)
              self.__fileBlurList.sort()
    ...
    def application(self):
        ...
        if(deblurring_file_path and os.path.exists(deblurring_file_path)):
            ...
            for i in range(iter_times):
                ...
                iter = self.iters[i]
                io.imsave(os.path.join(self.config.application.deblurring_result_dir, str(index)+'_'+infos[1]),deblur)
            print(f'file saved')
            ...
            for fileFullPath in self.__fileBlurList:
                ...
                for j in range(iter_times):
                    ...
                    deblur = (deblur * 255).astype('uint8')
                    iter = self.iters[j]
                    io.imsave(os.path.join(self.config.application.deblurring_result_dir, str(index)+'_'+infos),deblur)
                index += 1
                print(f'{index}/{self.data_length} done!')
            self.avg_time = self.runtime_sum / self.data_length #
            print(f'avg_time:  {self.avg_time:4.3f}')
            print(f'all saved')
        else:
